...
class cl_ptr:
    ...
    def __call__(self, match, seq_len):
        with tf.variable_scope(self.scope):
            d_match = dropout(match, keep_prob=self.keep_prob,
                              is_train=self.is_train)
            initial_state = self.gru.zero_state(self.batch, dtype=tf.float32)
            # _, state = self.gru(d_match, initial_state)
            _, state = tf.nn.dynamic_rnn(self.gru, d_match, seq_len, initial_state)
            return state
